...
func EnsureInitialImport(db IndexerDb, genesis sdk.Genesis) (bool, error) {
...
func checkGenesisHash(db IndexerDb, gh sdk.Digest) error {
	network, err := db.GetNetworkState()
	if errors.Is(err, ErrorNotInitialized) {
